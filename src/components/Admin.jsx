  ...
  return (
    <>
      ...
      >
        {!wannaBeAdmin
        && <>
          <FormControlLabel control={<Checkbox onChange={(event) => setWannaBeAdmin(event.target.value)} />} label="Are you an admin? 🤨" />
        </>
        }
        {wannaBeAdmin
        && <>
          <TextField id="admin" label="Admin Password" type="password" onChange={(event) => setAnswer(event.target.value)} />
